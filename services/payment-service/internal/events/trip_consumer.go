...
func NewTripConsumer(rabbitmq *messaging.RabbitMQ, service domain.Service, m *metrics.Metrics) *TripConsumer {
	return &TripConsumer{
		rabbitmq: rabbitmq,
		service:  service,
		metrics:  m,
...
func (c *TripConsumer) Listen() error {
	return c.rabbitmq.ConsumeMessages(messaging.PaymentTripResponseQueue, func(ctx context.Context, msg amqp091.Delivery) error {
		...
		if c.metrics != nil {
			c.metrics.RecordMessageConsumed(messaging.PaymentTripResponseQueue, "success", time.Since(start), msg.RoutingKey)
		}
